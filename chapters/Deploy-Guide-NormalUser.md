# Deploy Guide — Normal User

**FinanceFlow Workshop — OpenShift Container Capabilities**

Chapter-wise, copy-paste commands to run all 51 steps of the workshop as a
**normal (non-admin) user**, once cluster-admin has done the one-time setup.
...
- Cluster-admin has already run
  [Deploy-Guide-ClusterAdmin.md](Deploy-Guide-ClusterAdmin.md) Part A
  (cluster prep) and Part C (granting you self-service access — your own
  namespace or self-provisioner rights, plus the cluster-scoped
  SCC/ClusterRole/ClusterRoleBinding pre-applied on your behalf). If you're
  not sure this happened, ask your instructor before continuing.
- `oc` CLI installed and logged in: `oc whoami`
- Repo cloned locally, `cd chapters` before running anything below (all
  paths in this guide assume that working directory)
...
(e.g. `financeflow-<yourname>`), use that instead.

---

## Workshop Deployment (51 steps)

Each step below corresponds 1:1 to a step in `deploy-demo-resume.sh` — run
`./deploy-demo-resume.sh --list` to see the same 51 titles/numbers if you'd
rather jump around by name.

### Chapter 0 — Namespace

**Step 1 — Create the namespace**

Skip the `oc new-project` command if cluster-admin already created your
namespace for you — just switch into it:
...
If you're going to run the Chapter 6 CI/CD steps later, commit this now so
git matches what you just built:
...
> The `SecurityContextConstraints` and `ClusterRole` in this step are
> cluster-scoped — you cannot create them yourself. Ask your instructor to
> confirm they already applied `scc-financeflow.yaml` and
> `clusterrole-use-financeflow-scc.yaml` cluster-wide (see
> [Deploy-Guide-ClusterAdmin.md](Deploy-Guide-ClusterAdmin.md) Part C2)
> before you run this step — if they haven't, the first two commands below
> will fail with `forbidden`. The Role/RoleBinding commands are namespaced
> and will work fine either way.
...
The Sail Operator was already installed by cluster-admin — this step just
creates the `Istio`/`IstioCNI` custom resources, which is namespace-scoped
enough for a project admin to do:
...
> The `ClusterRoleBinding` here is cluster-scoped — same caveat as Step 18.
> Confirm with your instructor that
> `clusterrolebinding-kiali-monitoring.yaml` was already applied cluster-wide
...
Should contain `enableUserWorkload: true`. Cluster-admin already did this —
this is just a read-only check.

**Step 28 — Grant the monitoring SA scrape permission, apply the sidecar PodMonitor**

This is namespace-scoped and works fine as long as you have `admin` (not
`edit`) on your own namespace — see
[Deploy-Guide-ClusterAdmin.md](Deploy-Guide-ClusterAdmin.md) Part C1:
...
Look for `"result":[{` in the output. If empty, wait ~30–60s (scrape
interval is 15s) and re-check before assuming Step 28 failed.

---

### Chapter 6 — CI/CD

> Needs a GitHub fork of this repo and a Personal Access Token (repo
> scope). Skip this chapter if you're only doing a local demo.
...
Should list both `pipelines-console-plugin` and `gitops-plugin`. Already
done by cluster-admin — this is just a read-only check.

**Step 33 — Grant `financeflow-cicd` permission to push images and run buildah**

Requires `admin` on your namespace (see Part C1 of the cluster-admin
guide):
...
> MustRunAs` auto-derives a writable group for the shared pipeline-source
> PVC from your namespace's allocated range.
...
using the Route from Step 38 and the secret from Step 34.
...
Scoped to its own `grafana` namespace — not part of the cluster-admin
operator batch:
...
Or let the script track it for you:

```bash
./deploy-demo-resume.sh --from <step number or step-title text>
```

skips every step before your chosen start point.
